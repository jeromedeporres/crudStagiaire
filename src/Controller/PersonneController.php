...
class PersonneController extends AbstractController
{
    ...
    */
    public function index(FullnameService $fullnameService): Response
    {
        ...
        foreach ($personnes as $p ) {
            $p->nomComplet = $fullnameService->getFullname(
    ...
    *@Route("/personne/add2", name="personne_add2", methods={"GET", "POST"})
    ...
    {
        $personne = new Personne();
        $form = $this->createForm(PersonneType2::class, $personne);
        $form->handleRequest($request);
        if ($form->isSubmitted() && $form->isValid()) {
            $personne = $form->getData();
            $em = $this->getDoctrine()->getManager();
        ...
        return $this->render('personne/add2.html.twig', [
            'form' => $form->createView(),
        ]);
    }
    /**
